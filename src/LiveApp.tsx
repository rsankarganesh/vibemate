...
export default function LiveApp() {
  ...
  if (vibe) {
    ...
    return <><Header back /><main className="page vibe-page">
      <section className="vibe-hero"><div className="vibe-title"><span>{vibe.emoji}</span><div><p className="eyebrow">{vibe.type}</p><h1>{vibe.name}</h1></div></div><p>{vibe.when} · {vibe.location}</p>{vibe.inviteToken ? <button className="share" onClick={async () => { try { await navigator.clipboard.writeText(`${location.origin}${location.pathname}#/join/${vibe.inviteToken}`); notify('Invite link copied ✓'); } catch { notify('Couldn’t copy the invite. Allow clipboard access and try again.'); } }}><Share2 /> Copy invite</button> : <p className="helper">Ask the vibe’s creator for an invite link.</p>}</section><VibeTabs id={vibe.id}/>
      {!['expenses', 'settle'].includes(route.page) && <section className="balance-feature"><p>Your balance</p><h2>{me.balanceCents === 0 ? 'You’re all square' : `${me.balanceCents > 0 ? 'You’re owed' : 'You owe'} ${formatMoney(Math.abs(me.balanceCents))}`}</h2><div><span><small>You paid</small><strong>{formatMoney(vibe.expenses.filter(expense => !expense.deletedAt && expense.paidBy === 'alex').reduce((sum, expense) => sum + expense.amountCents, 0))}</strong></span><span><small>Fair share</small><strong>{formatMoney(me.shareCents)}</strong></span></div></section>}
      <button className="primary full add-expense" onClick={() => setModal(true)}><Plus /> Add expense</button>
      <section hidden={route.page === 'settle'}><div className="section-heading"><h2>Expenses</h2></div><input aria-label="Search expenses" placeholder="Search expenses or categories" value={query} onChange={event => setQuery(event.target.value)}/><div className="expense-list">{vibe.expenses.filter((expense) => !expense.deletedAt && `${expense.description} ${expense.category}`.toLowerCase().includes(query.toLowerCase())).reverse().map((expense) => <article className="expense-row" key={expense.id}>
      ...
      {route.page !== 'expenses' && <SettlementPlanner key={vibe.id} vibe={vibe} onRecord={setPayment}/>}
    </main>

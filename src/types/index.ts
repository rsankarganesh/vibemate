...
export type Vibe={id:string;name:string;emoji:string;type:string;when:string;startsAt:string;location:string;description:string;currency:string;maxMembers:number;status:'active'|'settled'|'archived';currentMemberId?:string;inviteToken?:string;members:Member[];expenses:Expense[];settlements:Settlement[];activity:Activity[]};
